<template>
  <UiCenter>
        ...
        >
          ...
          <div class="flex flex-wrap gap-2">
            <UiButton
              class="text-sm"
              :variant="
                route.query[option.name] === value ? 'highlight' : 'outline'
              "
    ...
    <ProductRecommendations
      :model-value="data.product.id"
      v-if="data?.product.id"
    />
  </UiCenter>
</template>
...
async function addCartLine() {
  ...
  try {
    ...
    if (
      ...
      currentVariant.value.node.availableForSale
    ) {
...
function isOptionDisabled(currentValue: string, option: OptionModel) {
  ...
  if (data.value) {
    const possibilities = Object.entries(currentQuery).filter(
      ([key, value]) => {
        if (data.value) {
          return data.value.product.options.find(
            (option) =>
              option.name === key && option.values.includes(value as string),
          );
        }
      },
    );
    ...
    const combination = combinations.value?.find((combination) =>
      possibilities.every(
        ([key, value]) =>
          combination.options[key] === value && combination.availableForSale,
      ),
    );
